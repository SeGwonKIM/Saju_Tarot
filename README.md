...
> 옛 저장소는 **동작하는 백업**으로 그대로 둡니다 — 지금 돌고 있는 서비스도 그쪽입니다.
>
> **여기서 할 일** — 계산 결과(원국·오행·타로)를 먼저 보여주고 풀이 문장은 나중에 채우는 구조로 바꾼다.
> 계산은 49ms, 풀이는 14초다. 지금은 그 14초 동안 화면이 멈춰 있다.
...
손님의 이름·생년월일이 들어 있는 DB 라 사본을 따로 둔다.

```powershell
.\백업.ps1                    # 백업 만들기
.\백업.ps1 -목록               # 목록 보기
.\백업.ps1 -검증 <파일>        # 진짜 열리는지 확인
.\백업.ps1 -복원 <파일>        # 새 폴더로 꺼내기 (덮어쓰지 않음)
```

- 백업은 `backups/` 에 **암호화되어**(AES-256-GCM) 쌓이고, 28일 지나면 지워진다
- 암호는 물어보거나 `BACKUP_PASSPHRASE` 로 준다. **이 컴퓨터에 저장하지 말 것** — 비밀번호 관리자에
- 서버가 돌고 있어도 안전하다 (SQLite 백업 API 를 쓴다)

**두 가지는 사람이 해야 한다**
- 만들어진 `.saju.enc` 를 **다른 장소에도 한 벌** 두기 (외장하드·클라우드). 원본과 같은 곳에만 두면 함께 잃는다
- **분기에 한 번 `-검증`** 으로 열어 보기. 열어본 적 없는 백업은 백업이 아니다
